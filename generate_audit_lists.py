...
def read_yolo_annotations(file_path):
    """读取YOLO格式的标注文件 返回: list of [class_id, x_center, y_center, width, height, confidence(可选)].
    """
    annotations = []
...
def calculate_iou(box1, box2):
    """计算两个YOLO格式框的IoU box: [x_center, y_center, width, height].
    """
...
def generate_audit_lists(true_labels_dir, pred_labels_dir, output_dir, conf_threshold=0.7, iou_threshold=0.5):
    ...
    # 统计信息
    stats = {"total_images": 0, "processed": 0, "missed_found": 0, "hard_found": 0, "wrong_label_found": 0}
