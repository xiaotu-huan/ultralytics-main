...
class ASFF(nn.Module):
    def __init__(self, level, in_channels_list, out_channels):
        """
        ASFF 模块：自适应空间特征融合
        :param level: 当前处理的层级（0=P3, 1=P4, 2=P5）
        :param in_channels_list: 三个输入特征图的通道数列表
        ...
        for _ in range(3):
            self.weight_convs.append(nn.Conv2d(out_channels, 1, 1, 1, 0))

    def forward(self, x_list):
        # 尺寸对齐
        target_h, target_w = x_list[self.level].shape[2:]
        adjusted_x = []
        for i, x in enumerate(x_list):
            x_conv = self.convs[i](x)
            x_resized = F.interpolate(x_conv, size=(target_h, target_w), mode="bilinear", align_corners=False)
            adjusted_x.append(x_resized)

        # 权重生成
        weight_logits = [self.weight_convs[i](adjusted_x[i]) for i in range(3)]
        weight_logits = torch.cat(weight_logits, dim=1)
        weights = F.softmax(weight_logits, dim=1)

        # 加权融合
        fused = sum(adjusted_x[i] * weights[:, i : i + 1, :, :] for i in range(3))
        return fused
