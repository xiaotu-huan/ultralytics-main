...
from .asff import ASFF  # 导入ASFF模块


from .dyhead import DyHead  # 导入 DyHead 模块
...
class ASFFDetect(nn.Module):
    def __init__(self, nc, *args, **kwargs):
        ...
        self.ch_list = [64, 128, 256]

        self.asff_p3 = ASFF(level=0, in_channels_list=self.ch_list, out_channels=64)
        self.asff_p4 = ASFF(level=1, in_channels_list=self.ch_list, out_channels=64)
        self.asff_p5 = ASFF(level=2, in_channels_list=self.ch_list, out_channels=64)

        self.detect = Detect(nc=self.nc, ch=[64, 64, 64])
        self.stride = self.detect.stride
        self.reg_max = self.detect.reg_max
        self.nl = self.detect.nl

    def forward(self, x):
        assert len(x) == 3, f"ASFFDetect 需输入 3 个特征图，实际接收 {len(x)} 个"
        p3, p4, p5 = x

        p3_fused = self.asff_p3([p3, p4, p5])
        p4_fused = self.asff_p4([p3, p4, p5])
        p5_fused = self.asff_p5([p3, p4, p5])

        return self.detect([p3_fused, p4_fused, p5_fused])
    



class DyHeadDetect(nn.Module):
    """基于官方Detect的DyHead检测头"""
    ...
    def __init__(self, nc=80, ch=()):
        ...
        # 关键：在官方Detect结构前添加DyHead模块
        self.dyhead_blocks = nn.ModuleList([DyHead(c) for c in ch])
    ...
    def forward(self, x):
        """前向传播 - 仅在官方Detect前添加DyHead"""
        # 对每个检测层的特征应用DyHead
        for i in range(self.nl):
            x[i] = self.dyhead_blocks[i](x[i])
            x[i] = torch.cat((self.cv2[i](x[i]), self.cv3[i](x[i])), 1)
