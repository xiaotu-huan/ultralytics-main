...
for xml_file in os.listdir("datasets/visdrone/train/labels"):
    if xml_file.endswith(".xml"):
        ...
        convert(
            f"datasets/visdrone/train/labels/{xml_file}",
            f"datasets/visdrone/train/labels/{img_name}.txt"
        )
...
for xml_file in os.listdir("datasets/visdrone/val/labels"):
    if xml_file.endswith(".xml"):
        img_name = os.path.splitext(xml_file)[0]
        convert(
            f"datasets/visdrone/val/labels/{xml_file}",
            f"datasets/visdrone/val/labels/{img_name}.txt"
        )